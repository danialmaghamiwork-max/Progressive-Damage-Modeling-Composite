function [FPF_load, LPF_load] = progressive_failure_max_stress(A_init, B_init, D_init, angles_deg, thickness, z_bottoms, z_tops, z_mids, angle_materials, has_core, core_thickness, layer_numbers)
    ...
    disp('شروع تحلیل progressive damage با حداکثر تنش محوری...');
    while true
        ...
        failure_results = max_on_axis_stress_failure(sigma1_mid, sigma2_mid, tau12_mid, Xt_layers, Xc_layers, Yt_layers, Yc_layers, S_layers, layer_numbers);
        ...
        for k = 1:n_layers
            if ~failed_layers(k) && failure_results{k}{1}
                ...
                elseif strcmp(mode, 'Matrix failure')
                    mat_k.Ey = mat_k.Ey / 10000;
                    mat_k.nux = mat_k.nux / 10000;
                    mat_k.Es = mat_k.Es / 10000;
                elseif strcmp(mode, 'Shear failure')
                    mat_k.Es = mat_k.Es / 10000;
                end
